...
import iattendsponsorActions from "../../redux/iattendSponsor/actions";
...
const { IATTENDSPONSOR_SPONSOR_REQUEST } = iattendsponsorActions;
...
class GitSearch extends Component {
	...
	componentDidMount(){
		...
		if(data !== "undefined"){
		  ...
		  this.props.IATTENDSPONSOR_SPONSOR_REQUEST(
			{
  ...
  toggle = () =>  {
    ...
    this.props.history.push({
      pathname: "/dashboard/eventlist-sponsor-detail",
      state: { 
	...
	body(){
		...
		return(
			...
			this.state.data.map((item, index, key)=>(
				...
				>
					<Avatar alt="Remy Sharp" src={'https://eventapp.dexagroup.com/apiget_file?fd='+item.logo} />
					<ListItemText className="borderList">
					<Typography className="my_approval_leaverequest"><b>Name : </b>{item.name}</Typography>
					{/* <Typography className="other_applications_my_ticket_description">{item.description ? item.description : '-'}</Typography> */}
	...
	render() {
		...
		return (
            ...
            <Icon className="eventJudulAgendaIcon" onClick={() => this.onclickBack()}>arrow_back</Icon> <h1 className="eventJudulAgenda">Sponsor List</h1>
...
export default compose(connect(
	state => ({
	  result:state.IattendSponsorRequest
	}),
	{IATTENDSPONSOR_SPONSOR_REQUEST}
